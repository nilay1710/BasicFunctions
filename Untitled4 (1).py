...
def drawLine(img,x,y,slope,b=0,g=0,r=255,thickness = 2):
    height = img.shape[0]
    width = img.shape[1]
    points = []
...
#given 4 points (p = [x,y]) and image, it crops the image with those 4 lines
...
def cropImage(img, p1,p2,p3,p4):
    ...
    cropLine(img,x0,y0,points[0][0],points[0][1],points[1][0],points[1][1])
    cropLine(img,x0,y0,points[1][0],points[1][1],points[2][0],points[2][1])
    cropLine(img,x0,y0,points[2][0],points[2][1],points[3][0],points[3][1])
    cropLine(img,x0,y0,points[3][0],points[3][1],points[0][0],points[0][1])
...
img_arr = cv2.imread('smiley.png', 1)
cropImage(img_arr, [10,10],[200,10],[200,100],[10,100])
cv2.imwrite('color_img2.jpg', img_arr)
...
# given a list of points (points = [[x1,y1],[x2,y2]...]) and angle, it rotates the points in the same list
